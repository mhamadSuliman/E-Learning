...
class ExamAttemptController extends Controller
{
    ...
    public function submitAnswer(
        ...
        if ($attempt->exam_id != $exam_id) {
            return response()->json([
                'message' => 'المحاولة لا تتبع هذا الامتحان ❌'
            ], 400);
        }
        if ($attempt->exam->course_id != $course_id) {
            return response()->json([
                'message' => 'الامتحان لا يتبع هذا الكورس ❌'
            ], 400);
        }
    ...
    public function submitAttempt(
        ...
        $attempt = ExamAttempt::with('exam','answers')->find($attempt_id);
